...
Ly = sim_par.Ly;                  
xi = sim_par.xi;                                       % coordinate of excitation (normalised)
yi = sim_par.yi;

% diffuser parameters
dx = diff_par.dx;                                       % normalised coordinate of diffuser center (dx - x coordinate)
dy = diff_par.dy;                                                
maxwd = diff_par.maxwd;                                 % maximum QRD well depth
...
li = floor(xi*N);                                      % grid indices of excitation
mi = floor(yi*M);
...
if diff_opts.diffuser_on
    if strcmp(sim_opts.diffuser_type,'plane')                  
        D(ceil(H/2),2:nw*w-1) = 0;
    else
    ...
    d(dbottom:dtop,dstartx:dendx) = D;
end
...
% 3.2 excitation
% 3.2.1 source position determined by angle of incidence 
...
li = sp(1);                                                       % update the source position coordinate
mi = sp(2);
...
%          u1(mi,li) = u1(mi,li)+x(n);
    ...
    if sim_opts.plot_on
        if mod(n,100)==0
            han.CData = 20*gather(u0);
            title(sprintf('simulation duration = %f (s), n=%d out of %d\n',dur,n,Nf));
            drawnow
        end
    end
